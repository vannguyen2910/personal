 ...
 *
 * Drop this into a deck, mockup, or page wherever you want to supply an
 * image. You control the slot's shape and size; fill it by dragging an
 * image file onto it, or by clicking to browse. The fill happens entirely
 * client-side (resized/re-encoded to WebP via canvas) — there's no server,
 * so a filled slot only lasts for the current page session and is lost on
 * reload. For a screenshot you want to keep permanently, either bake it in
 * by setting the `src` attribute on the element in the HTML, or wire up
 * your own persistence: this component will read an initial value from a
 * same-origin `.image-slots.state.json` file (`{ "<id>": "data:image/..." }`)
 * if one is served alongside the page, and write to it if a
 * `window.omelette.writeFile(path, contents)` function is present on the
 * page — otherwise it just no-ops on save.
 *
 * Attributes:
